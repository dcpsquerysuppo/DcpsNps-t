...
public List checkSevaarthIdExist(String lStrSevaarthId,String lStrDDOCode, String strEmpname, String strDcpsId, String strPpanNo) {
	...
	sb.append(" SELECT reg_status, nvl(to_char(EMP_SERVEND_DT,'dd-MM-yyyy'),'01-01-1900'), nvl(AC_DCPS_MAINTAINED_BY,'NA'),nvl(to_char(DOJ,'dd-MM-yyyy'),'01-01-1900'),nvl(PRAN_NO,'#'),nvl(ddo_code,0) FROM mst_dcps_emp where form_status=1  ");
...
////$t12Jun2023
public int checkPranExistInAttach(String searchPran) {
	List lLstEmpFiveInst = null;
	StringBuilder lSBQuery = null;
		try {
			lSBQuery = new StringBuilder();
			lSBQuery.append(" SELECT nvl(count(pran_no),0) FROM mst_dcps_emp where pran_no ='"+searchPran.trim()+"' ");
