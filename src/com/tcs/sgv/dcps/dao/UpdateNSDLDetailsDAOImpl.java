...
public class UpdateNSDLDetailsDAOImpl extends GenericDaoHibernateImpl implements UpdateNSDLDetailsDAO {
	...
	public UpdateNSDLDetailsDAOImpl(Class type, SessionFactory sessionFactory) {
...
//$t 2020 17-1  2020 year added
//$t 31-12-2020 2021 year added
...
//end
	...
	public String getDtoRegNo(String treasuryCode) {
	  ...
	  public String updateTransactionDetails(String fileName, String tranIds){
	    List temp = null;
	    String data = "";
        String status="N"; 
...
/////$t 13-05-2021 transaction_update_date=sysdate
	    if ((count == 7) || (count == 8)) {
	      ...
	      //Strblds.append(" update NSDL_BH_DTLS set TRANSACTION_ID='" + tranIds + "', FILE_STATUS=11, STATUS=1 , REMARK_FOR_TRAN_ID_UPDATE=null,transaction_update_date=sysdate WHERE FILE_NAME='" + fileName + "'");
	    ...
	    else
	    {
	      ...
	      //Strblds.append(" update NSDL_BH_DTLS set TRANSACTION_ID='" + tranIds + "', FILE_STATUS=11, STATUS=1,transaction_update_date=sysdate WHERE FILE_NAME='" + fileName + "' ");
	      this.logger.info("Query to getErrorData in  heaqder**********" + Strblds.toString());
	      SQLQuery lQuerys = this.ghibSession.createSQLQuery(Strblds.toString());
	  ...
	  public void updateTransactionDetailsRATF(String fileName, String tranIds)
	  {
	    List temp = null;
	    String data = "";
	    ...
	    Strbldx.append(" select file_Status from RAT_nsdl_bh_dtls where file_name='" + fileName+"'");
	    SQLQuery lQueryx = this.ghibSession.createSQLQuery(Strbldx.toString());
	    int count = Integer.parseInt(lQueryx.uniqueResult().toString());
	    this.logger.info("Query to getErrorData in  heaqder**********" + Strbldx.toString());
/////$t 13-05-2021 transaction_update_date=sysdate
	    if ((count == 7) || (count == 8)) {
	      ...
	      Strblds.append(" update RAT_NSDL_BH_DTLS set TRANSACTION_ID='" + tranIds + "', FILE_STATUS=11, STATUS=1 , REMARK_FOR_TRAN_ID_UPDATE=null,transaction_update_date=sysdate WHERE FILE_NAME='" + fileName + "'");
	      this.logger.info("Query to getErrorData in  heaqder**********" + Strblds.toString());
	      SQLQuery lQuerys = this.ghibSession.createSQLQuery(Strblds.toString());
	    ...
	    else
	    {
	      ...
	      Strblds.append(" update RAT_NSDL_BH_DTLS set TRANSACTION_ID='" + tranIds + "', FILE_STATUS=11, STATUS=1,transaction_update_date=sysdate WHERE FILE_NAME='" + fileName + "' ");
	      this.logger.info("Query to getErrorData in  heaqder**********" + Strblds.toString());
	      SQLQuery lQuerys = this.ghibSession.createSQLQuery(Strblds.toString());
		 ...
		 public List getAllDataRATF(String yrCode, String month, String strLocationCode)
		    throws Exception
		  {
		    ...
		    try {
		      lSBQuery = new StringBuilder();
		      lSBQuery.append("SELECT distinct bh.FILE_NAME,bh.BH_EMP_AMOUNT,bh.BH_EMPLR_AMOUNT,bh.TRANSACTION_ID,bh.file_status  FROM RAT_NSDL_BH_DTLS bh where bh.YEAR='" + yrCode + "' and bh.MONTH='" + month + "' and (( bh.FILE_STATUS=1 and bh.TRANSACTION_ID is null) or ( bh.FILE_STATUS in (7,8) and bh.TRANSACTION_ID is not null) ) and bh.STATUS<>-1 and bh.file_name like '" + strLocationCode + "%'   ");
		...
		public int checkTranAlreadyMapped(String searchTransaction) {
			List lLstEmpFiveInst = null;
			StringBuilder lSBQuery = null;
				try {
					lSBQuery = new StringBuilder();
					lSBQuery.append(" SELECT nvl(count(TRANSACTION_ID),0) FROM NSDL_BH_DTLS where FILE_STATUS='11' and status='1' and TRANSACTION_ID ='"+searchTransaction.trim()+"' ");
			...
			return (int) lLstEmpFiveInst.get(0);
		}
