...
//Added by Ashish for Contribution already done data			
	public List getContriOfEmpForSelectedPeriod(Long dcpsEmpId,
		...
		System.out.println(dt1.format(contriStartDate));
		System.out.println(dt1.format(contriEndDate));
        
        try {/////$t2Jun2023
            final Session ghibSession = (Session)ServiceLocator.getServiceLocator().getSessionFactorySlave().getCurrentSession();
            sb = new StringBuilder();
            sb.append(" select trn.voucher_no,to_char(trn.voucher_date,'dd-MM-yyyy'),trn.contribution,nvl(trn.STATUS,'M'),trn.CONTRIBUTION_EMPLR,to_char(trn.STARTDATE,'dd/MM/yyyy'), ");
            sb.append(" to_char(trn.ENDDATE,'dd/MM/yyyy'),look.lookup_name,look.lookup_id FROM Trn_Dcps_Contribution trn join RLT_DCPS_PAYROLL_EMP rlt ON trn.DCPS_EMP_ID=rlt.DCPS_EMP_ID INNER JOIN CMN_LOOKUP_MST look on ");
            sb.append(" look.lookup_id = RLT.GIS_APPLICABLE WHERE trn.dcps_Emp_Id='"+dcpsEmpId.toString().trim()+"' AND (('"+dt1.format(contriStartDate)+"' between trn.startDate and trn.endDate)OR ('"+dt1.format(contriEndDate)+"' between trn.startDate and trn.endDate)) ");
            if (typeOfPayment.equals("700046") || typeOfPayment.equals("700047"))
            sb.append(" AND (type_Of_Payment = '700046' OR type_Of_Payment = '700047') ");
            else
            sb.append(" AND trn.type_Of_Payment = '"+typeOfPayment.trim()+"' ");
            
            sb.append(" AND trn.ddo_Code = '"+lStrDDOCode.trim()+"' AND trn.reg_Status in (1,3,4,2) ");
            
            final Query stQuery = (Query)ghibSession.createSQLQuery(sb.toString());
            lListTrnDcpsContribution = stQuery.list();
        }
        catch (Exception e) {
            e.printStackTrace();
        }
