...
public class NpsPendencyTrackingDAOImpl extends GenericDaoHibernateImpl
  implements NsdlNpsDAO
{
  private final Log gLogger = LogFactory.getLog(getClass());
  Session ghibSession = null;
  
  public NpsPendencyTrackingDAOImpl(Class type, SessionFactory sessionFactory)
  {
    super(type);
  ...
  public String getRoleId(Long lLngPostId) {
		...
		try {
			StringBuilder lSBQuery = new StringBuilder();
			lSBQuery.append(" select ROLE_ID  from ACL_POSTROLE_RLT where POST_ID ='"+ lLngPostId + "'  and ROLE_ID in (700003,700001,700004,700002,700016) ");
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
