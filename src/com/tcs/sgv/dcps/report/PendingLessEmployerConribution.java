		...
		if (loginDetail.get("userId")!=null && !"".equalsIgnoreCase(loginDetail.get("userId").toString()))
			...
			if (report.getReportCode().equals("9000522")) 
			{
				...
				for (int i=0;i<TableData1.size();i++)
				{
					...
					if (tupleSub[3] != null) 
					{
						 int count =chkLessEmplrBillGeneratedCountTreasury(lStrYearId,SubTreasury);
			 ...
			 if (report.getReportCode().equals("9000523")) {
				...
				for (int i=0;i<TableData1.size();i++)
				{
					...
					if (tupleSub[5] != null) 
					{
						count =chkLessEmplrBillGeneratedCountDDO(lStrYearId,SubTreasury,tupleSub[1].toString());/////$t20July2023
						td.add(new URLData(count ,"hrms.htm?actionFlag=reportService&reportCode=9000524&action=generateReport&DirectReport=TRUE&displayOK=FALSE&finYear="+lStrYearId+"&ddoCode="+tupleSub[1]));
...
//					td.add(i+1);
					...
					if(tupleSub[19].toString().equalsIgnoreCase("STDMPWM8901"))
					{
						int a=100;
					}
...
//							Diff=Double.parseDouble(tupleSub[22].toString())-1;
						...
						for(int D=0;D<Diff;D++)
						{
							...
							if(FromDateyear<=2018 )  // for  10% 
								...
								remaining=Math.ceil(remaining+(Newcontribution*Per)/10);
							else if((FromDateyear<=2018)||(ToDateyear==2019 && ToDateMonth>=4 ))  // for  14% /////$t20July2023 
								remaining=Math.ceil(remaining+(Newcontribution*10)/10);
							FromDateMonth++;
...
//								Diff=Double.parseDouble(tupleSub[22].toString())-1;
							...
							for(int D=0;D<Diff;D++)
							{
								...
								if(FromDateyear<=2018 )  // for  10% 
									...
									remaining=Math.ceil(remaining+(Newcontribution*14)/10);
								else if((FromDateyear<=2018)||(ToDateyear==2019 && ToDateMonth>=4 ))  // for  10% /////$t20July2023 
									remaining=Math.ceil(remaining+(Newcontribution*10)/10);
								FromDateMonth++;
						...
						if(FromDateyear<=2018 && ToDateyear<=2018)  // for  10% 
							...
							remaining=(double) Math.floor(remaining+(contribution1*14)/10);
						else if((FromDateyear<=2018)||(ToDateyear==2019 && ToDateMonth>=4 ))  // for  14% /////$t20July2023 
							remaining=Math.ceil(remaining+(contribution1*10)/10);
							}
...
//							 Pending=Pending-Long.parseLong(fourPercent);
						 ...
						 if (chkLessEmplrBillGenerated(tupleSub[19].toString(),Pending) > 0){
...
//get Treasury Wise Pending Details
	...
	private List getDDOWisePendingDtls(String locationId, String lStrYearId) {
		...
		try
		{
			...
			sb.append(" treasury,C_ddocode,C_DDO_NAME,sum(TOTAL_EMPLOYEE_CONTRIBUTION),sum(TOTAL_EMPLOYER_CONTRIBUTION),count(sevaarthid) ");/////$t27July2023
			sb.append(" FROM LESSEMPLOYER ");
			sb.append(" where year='"+lStrYearId+"' ");
			sb.append(" and substr(treasury,0,2 )=substr('"+locationId+"',0,2) ");
			sb.append(" and (TOTAL_EMPLOYEE_CONTRIBUTION)>(TOTAL_EMPLOYER_CONTRIBUTION) ");
			sb.append(" and dcps_flag='Y' ");
			sb.append(" group by substr(treasury,0,2 ),C_ddocode,C_DDO_NAME,treasury ");/////$t27July2023
	...
	private List getEmpWisePendingDtls(String locationId, String lStrYearId) {
		...
		try
		{
			...
			sb.append(" and less.C_DDOCODE='"+locationId+"' ");/////$t27July2023
			////sb.append(" and SEVAARTHID='STDMPWM8901' ");/////$t27July2023
...
public int chkLessEmplrBillGenerated(String sevarthID, Double pending) {
	
	StringBuilder sb = new StringBuilder();
	
	sb.append(" SELECT count(data.NPS_EMPLR_DIFFERENCE_ADJ) FROM mst_dcps_emp emp join HR_EIS_EMP_MST eis on emp.ORG_EMP_MST_ID=eis.EMP_MPG_ID join HR_PAY_PAYBILL hr on eis.emp_id=hr.emp_id join PAYBILL_HEAD_MPG head on ");
	sb.append(" hr.PAYBILL_GRP_ID=head.PAYBILL_ID join NSDL_PAYBILL_DATA data on head.PAYBILL_ID=data.PAYBILL_GRP_ID where data.NPS_EMPLR_DIFFERENCE_ADJ='"+pending+"' and head.APPROVE_FLAG ='1' and emp.sevarth_id='"+sevarthID+"' ");
	...
	return (int) lLstResult.get(0);
}

private int chkLessEmplrBillGeneratedCountTreasury(String lStrYearId, String subTreasury) {
	int count=0;
    StringBuilder sb = new StringBuilder();
	sb.append(" SELECT sevaarthid,amount FROM LESSEMPLOYER where year='"+lStrYearId+"' and substr(treasury,0,2 )=substr('"+subTreasury+"',0,2) and (TOTAL_EMPLOYEE_CONTRIBUTION)>TOTAL_EMPLOYER_CONTRIBUTION and dcps_flag='Y' ");
	////countPayrollEmployerB
	Session ghibSession = ServiceLocator.getServiceLocator().getSessionFactorySlave().getCurrentSession();
	Query lQuery = ghibSession.createSQLQuery(sb.toString());
	List lLstResult = lQuery.list();
	int countLessEmployer=0;
	int countPayrollEmployerB = 0;
	for (Iterator it = lLstResult.iterator(); it.hasNext();)
    {
     Object[] lObj = (Object[])it.next();
     System.out.println("list-->"+lObj[0].toString()+" --"+lObj[1].toString());
     countLessEmployer++;
     sb = new StringBuilder();
     sb.append(" SELECT count(data.NPS_EMPLR_DIFFERENCE_ADJ) FROM mst_dcps_emp emp join HR_EIS_EMP_MST eis on emp.ORG_EMP_MST_ID=eis.EMP_MPG_ID join HR_PAY_PAYBILL hr on eis.emp_id=hr.emp_id join PAYBILL_HEAD_MPG head on ");
 	 sb.append(" hr.PAYBILL_GRP_ID=head.PAYBILL_ID join NSDL_PAYBILL_DATA data on head.PAYBILL_ID=data.PAYBILL_GRP_ID where data.NPS_EMPLR_DIFFERENCE_ADJ='"+lObj[1].toString()+"' and data.NPS_EMPLR_DIFFERENCE_ADJ >0 and head.APPROVE_FLAG ='1' and emp.sevarth_id='"+lObj[0].toString()+"' ");
 	 //sb.append(" hr.PAYBILL_GRP_ID=head.PAYBILL_ID join NSDL_PAYBILL_DATA data on head.PAYBILL_ID=data.PAYBILL_GRP_ID where data.NPS_EMPLR_DIFFERENCE_ADJ='9428' and data.NPS_EMPLR_DIFFERENCE_ADJ >0 and head.APPROVE_FLAG ='1' and emp.sevarth_id in('HCDAHGF9501','HCDNGKM8601','HCDSADF7501','HCDURSM7603','HCDNSMM7501'); ");
	...
	int countPayrollEmployerA=(int) lLstResult.get(0);
	 if(countPayrollEmployerA>0)
		 countPayrollEmployerB++;
     }
	System.out.println("countLessEmployer-->"+countLessEmployer);
	System.out.println("countPayrollEmployer-->"+countPayrollEmployerB);
	
	return (int) countLessEmployer-countPayrollEmployerB;
}

private int chkLessEmplrBillGeneratedCountDDO(String lStrYearId, String subTreasury, String ddo) {
	int count=0;
    StringBuilder sb = new StringBuilder();
	sb.append(" SELECT sevaarthid,amount FROM LESSEMPLOYER where year='"+lStrYearId+"' and substr(treasury,0,2 )=substr('"+subTreasury+"',0,2) and C_ddocode='"+ddo+"' and (TOTAL_EMPLOYEE_CONTRIBUTION)>TOTAL_EMPLOYER_CONTRIBUTION and dcps_flag='Y' ");/////$t27July2023
	////countPayrollEmployerB
	Session ghibSession = ServiceLocator.getServiceLocator().getSessionFactorySlave().getCurrentSession();
	Query lQuery = ghibSession.createSQLQuery(sb.toString());
	List lLstResult = lQuery.list();
