...
public class MissingCreditTrackingReport extends DefaultReportDataFinder {
...
//            con = lObjSessionFactory.getCurrentSession().connection();
             con=DriverManager.getConnection("jdbc:db2://10.34.82.226:60000/IFMSMIGR","db2mahait", "Mahasevaarth@123");   // For live
			...
			gLogger.info("#################before Report code 9000325");
			
            if (report.getReportCode().equals("9000325")) {

            	

				gLogger.info("#################Report code 9000325");
                // report.setStyleList(noBorder);

                ArrayList rowList = new ArrayList();
                //setSessionInfo(inputMap);
                 lStrYearId = (String) report.getParameterValue("yearId");
				 lStrMonthId = (String) report.getParameterValue("monthId");
				
/*
                StringBuilder SBQueryQuerys = new StringBuilder();

                SBQueryQuerys.append("SELECT role_id FROM acl_postrole_rlt where post_id="+gLngPostId);
				
				
				String StrSqlQuerys = SBQueryQuerys.toString();
				ResultSet rss = smt.executeQuery(StrSqlQuerys);
				
				String roleId=null;
				     while (rss.next()) {
				    	 roleId=rs.getString(0);
				     }
                   
				     gLogger.info("#################role id"+roleId);
						*/
                
				gLogger.info("#################year id"+lStrYearId);
				
				
				yearIds=lStrYearId.split("-");
				if(Integer.parseInt(lStrMonthId)>=4){
					
					yearIdByMonth=yearIds[0];
				}else if(Integer.parseInt(lStrMonthId)<4){
					yearIdByMonth=yearIds[1];
				}
				
				gLogger.info("#############yearIdByMonth"+yearIdByMonth);
				gLogger.info("#################month id"+lStrMonthId);
...
inner join mst_dto_reg dto on dto.loc_id=loc.loc_id
           ...
           // --    inner join PAYBILL_HEAD_MPG head on head.LOC_ID=ddo.LOCATION_CODE 
            // --   inner join HR_PAY_PAYBILL paybill on paybill.PAYBILL_GRP_ID=head.PAYBILL_ID 
              ...
              if(locationId==10702){
            	  ...
            	  SBQueryQuery.append(" SELECT loc.loc_name,count(DCPS_CONTRIBUTION_ID),sum(contribution),loc.loc_id FROM trn_dcps_contribution trn ");
            	  SBQueryQuery.append(" inner join cmn_location_mst loc on substr(loc.loc_id,1,2)=substr(trn.treasury_code,1,2) and loc.department_id=100003 ");
            	  SBQueryQuery.append(" where trn.IS_MISSING_CREDIT='Y' and to_char(trn.created_date,'MM')="+lStrMonthId+" and to_char(trn.created_date,'YYYY')="+yearIdByMonth+" ");
            	  ...
            	  String locs_id=locationId.toString().substring(0,2); 
            	  SBQueryQuery.append(" SELECT loc.loc_name,count(DCPS_CONTRIBUTION_ID),sum(contribution),loc.loc_id FROM trn_dcps_contribution trn ");
             	  SBQueryQuery.append(" inner join cmn_location_mst loc on loc.loc_id=trn.treasury_code and (loc.department_id=100006 or loc.department_id=100003)");
             	  SBQueryQuery.append(" where trn.IS_MISSING_CREDIT='Y' and to_char(trn.created_date,'MM')="+lStrMonthId+" and to_char(trn.created_date,'YYYY')="+yearIdByMonth+" and trn.treasury_code like '"+locs_id+"%'");
             	  SBQueryQuery.append(" group by loc.loc_id,loc.loc_name ");
             	  SBQueryQuery.append(" order by loc.loc_id ");
              ...
              /*    if(Integer.parseInt(rptCd)==2){
                	...
                	//String loc_id=locationId.toString();
                	...
                	SBQueryQuery.append(" left join cmn_location_mst loc on loc.loc_id = substr(emp.ddo_code,1,4)  ");
                //	SBQueryQuery.append(" left join ORG_EMP_MST org on org.EMP_ID=emp.ORG_EMP_MST_ID left join org_userpost_rlt post on post.user_id=org.user_id  ");
                	SBQueryQuery.append(" where emp.reg_status=1 and emp.ddo_code <> '1111222222' and loc.loc_id like '"+loc_id+"%' and emp.form_status=1 and emp.dcps_or_gpf='Y' and emp.emp_servend_dt > sysdate "); //and post.activate_flag=1 
                	SBQueryQuery.append(" group by loc.loc_id,loc.loc_name  ");
                	SBQueryQuery.append(" order by loc.loc_id ");
          ...
          String urlPrefix1 = "ifms.htm?actionFlag=reportService&reportCode=9000345&action=generateReport";
          gLogger.info("#########urlPrefix "+urlPrefix1);
                StrSqlQuery = SBQueryQuery.toString();
                gLogger.info("#########StrSqlQuery "+StrSqlQuery);
               ...
               String amount=null;
                while (rs.next()) {
                    ...
                    gLogger.info("########### url pref"+urlPrefix1+"&treasuryCode="+rs.getString(4)+"&monthId="+lStrMonthId+"&yearId="+yearIdByMonth);
                    rowList.add(new StyledData(rs.getString(1), CenterAlignVO));
                    //rowList.add(new StyledData(rs.getString(2), CenterAlignVO));
                    rowList.add(new URLData(rs.getString(2), urlPrefix1+"&treasuryCode="+rs.getString(4)+"&monthId="+lStrMonthId+"&yearId="+yearIdByMonth));
                    rowList.add(new StyledData(rs.getString(3), CenterAlignVO));
                   
                    	gLogger.info("################ after rowlist add");
                    
                    	//	rowList.add(new URLData(rs.getString(3), urlPrefix1+"&treasuryCode="+rs.getString(1)+"&rptCd=1"));
            
                    	//	rowList.add(new URLData(rs.getString(1), urlPrefix1+"&treasuryCode="+rs.getString(1)+"&rptCd=2"));
                   
                    	//	rowList.add(new URLData(rs.getString(2), urlPrefix1+"&treasuryCode="+rs.getString(2)+"&rptCd=3"));
...
//			Session lObjSession = ServiceLocator.getServiceLocator().getSessionFactory().getCurrentSession();
			...
			String lStrBufLang = " select MONTH_ID ,MONTH_NAME  from SGVA_MONTH_MST where LANG_ID='en_US' order by MONTH_NO  ";
...
//			lObjQuery.setString("langId", lStrLangId);
	...
	public List getYear(String lStrLangId, String lStrLocId) {
		...
		try {
			...
			String lStrBufLang = " select fin_year_desc,fin_year_desc from SGVC_FIN_YEAR_MST where LANG_ID='en_US' and fin_year_code between '2007' and '2023' order by fin_year_code ";
