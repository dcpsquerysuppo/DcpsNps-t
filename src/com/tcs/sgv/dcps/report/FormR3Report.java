...
public class FormR3Report extends DefaultReportDataFinder implements ReportDataFinder
{
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria)throws ReportException
	{
		...
		try{
			...
			if (lObjReport.getReportCode().equals("700097")) 
			{
				...
				lStrDdoCode = (String)lObjReport.getParameterValue("ddoCode");
				...
				lStrYear = lObjDcpsCommonDAO.getFinYearForYearId(lLngYearId);
				lStrDdoName="";
				...
				/*if(payComm.equals("700016")){
					...
					rowList1.add(new StyledData("Basic Pay :" + sevenbasicPay + space(15) + "D.P. :"
							+ DP + space(15) + "D.A. :" + Math.round(daRate)
							,boldVO));
					dataList.add(rowList1);
				}*/
				...
				if (lLngYearId == 29L) {
		               openEmployeeInt = lOpeningEmpTier1 * 0.081D / 12.0D * 6.0D + lOpeningEmpTier1 * 0.08D / 12.0D * 6.0D;
		               openEmployerInt = lOpeningEmployrTier1 * 0.081D / 12.0D * 6.0D + lOpeningEmployrTier1 * 0.08D / 12.0D * 6.0D;
		            } else {
		               for(Integer lInt = 0; lInt < interestRates.size(); lInt = lInt + 1) {
		                  lObjIntDtls = (Object[])interestRates.get(lInt);
		                  lDoubleIntRate = Double.valueOf(lObjIntDtls[0].toString().trim());
		                  lDoubleIntRate = Round(lDoubleIntRate, 2);
		                  openEmployeeInt = openEmployeeInt + lDoubleIntRate * lOpeningEmpTier1 / 100.0D;
		                  openEmployerInt = openEmployerInt + lDoubleIntRate * lOpeningEmployrTier1 / 100.0D;
		               }
		            }
				...
				if ((lLstOpeningBalcTier1 != null) && (lLstOpeningBalcTier1.size() > 0))
				rowList.add(Round((openEmployeeInt),2)+Round(openEmployerInt,2));
				else
				rowList.add(Round((openEmployeeInt),2)+Round(openEmployerInt,2));
			...
			//jitu
			if (lObjReport.getReportCode().equals("800009573")) 
			{
				DateFormat dateFormat = new SimpleDateFormat("dd MMM,yyyy");
				DateFormat dateFormat2 = new SimpleDateFormat("yyyy-MM-dd");
				DateFormat dateFormat3 = new SimpleDateFormat("dd MMM yyyy");
				...
				if (lObjReport.getParameterValue("sevarthId") != null && !"".equalsIgnoreCase(lObjReport.getParameterValue("sevarthId").toString())) {
...
//				String lStrDcpsEmpId = (String)lObjReport.getParameterValue("dcpsEmpId");
				...
				String lStrTreasuryName = getLocationName(Long.valueOf(Long.parseLong(lStrDdoCode.substring(0,4))));
				if (lStrDdoCode!=null && !"".equalsIgnoreCase(lStrDdoCode))
				{
					 lStrDdoName = getDdoName(lStrDdoCode);
				}
				
				String lStrDcpsEmpId=getEmpDCPSEMPID(lStrSevaarthId);
				String lStrEmpName = getEmpName(Long.valueOf(Long.parseLong(lStrDcpsEmpId)));
				...
				if ((lLstOpeningBalcTier1 != null) && (lLstOpeningBalcTier1.size() > 0))
				{
					...
					if (lObj[0] != null)
						//lStrOpeningEmpTier1 = lObj[0].toString();
					    lStrOpeningEmpTier1 = lObj[1].toString();////$t27June2023
				...
				if(lLstDtlsTier2 != null && lLstDtlsTier2.size() > 0)
				{
					...
					if(lObj[6] != null){
						//lOpeningEmpTier1  =Double.parseDouble(lObj[6].toString());
						lOpeningEmpTier1  =Double.parseDouble(lObj[7].toString());////$t27June2023
					}
					if(lObj[7] != null){
						lOpeningEmployrTier1 =Double.parseDouble( lObj[7].toString());
					}
				...
				rowList.add("");
				//rowList.add(lStrOpeningEmpTier1);
				rowList.add(lStrOpeningEmployrTier1);////$t27June2023
				rowList.add(lStrOpeningEmployrTier1);
				if ((lLstOpeningBalcTier1 != null) && (lLstOpeningBalcTier1.size() > 0))
				rowList.add(Round((openEmployeeInt),2)+Round(openEmployerInt,2));
				else
				...
				//Double totalContr=Double.valueOf(lTotalAmt.doubleValue() + lTotalMissingamt.doubleValue()+ lLngYearlyContriTier2.doubleValue());
				Double totalContr=Double.valueOf(lTotalAmt.doubleValue() + lTotalMissingamt.doubleValue()+ lLngYearlyContriTier2.doubleValue());////$t27June2023
				//Double totalInt=Double.valueOf(lLngTotalIntrstT2.doubleValue() + empIntrstT2.doubleValue() +  empMissingIntrstT2.doubleValue());
				Double totalInt=Double.valueOf(empIntrstT2.doubleValue() +  empMissingIntrstT2.doubleValue());////$t27June2023
				Double totalEmplrInt=Double.valueOf(emplrIntrstT2.doubleValue() + emplrMissingIntrstT2.doubleValue() );
				...
				rowList.add("");
				//rowList.add(totalContr);
				rowList.add(Double.valueOf(lTotalAmt.doubleValue()+lTotalMissingamt.doubleValue()));////$t27June2023
				rowList.add(Double.valueOf(lTotalAmt.doubleValue()+lTotalMissingamt.doubleValue()));
				rowList.add("");
				lLstDataList.add(rowList);
				...
				//Double totalEmpAmt=Double.valueOf( lOpeningEmpTier1.doubleValue() + totalInt.doubleValue() + totalContr.doubleValue() );
				Double totalEmplyrAmt=Double.valueOf(lTotalAmt.doubleValue() +  lTotalMissingamt.doubleValue() + lOpeningEmployrTier1.doubleValue() + totalEmplrInt.doubleValue() );
				Double totalEmpAmt=totalEmplyrAmt;////$t27June2023
	...
	public List getAllEmpsUnderDDO(String lStrddoCode, String lStrLangId,
		...
		try {
			if (!lStrddoCode.equals("-1")) {
				...
				sb.append(" WHERE EM.ddoCode = :ddoCode  order by EM.name");
	...
	public List getEmpAllDtlsTire1(Long lLngDcpsEmpId, Long lLngYearId, String lStrType,String fromDate,String toDate)
	{
		...
		try{
			...
			else
			{
				...
				lSBQuery.append(" WHERE   trn.MONTH_ID <> 3 and trn.fin_year_id="+lLngYearId+" and trn.DCPS_EMP_ID ="+lLngDcpsEmpId+"  and  trn.TYPE_OF_PAYMENT="+lStrType+" ");
			}

			lSBQuery.append("  AND trn.REG_STATUS = 1  and  trn.EMPLOYER_CONTRI_FLAG = 'Y'  and trn.is_missing_credit is null  ");
	...
	public List getEmpAllMissingCreditTire1(Long lLngDcpsEmpId,String fromDate,String toDate,Long lLngYearId)
	{
		...
		try{
			...
			lSBQuery.append("AND trn.startdate between '2005-04-01' and '2015-03-31' "); //jitu 15_DEC_2022
	...
	/*public List getEmployerAllDtlsTire1(Long lLngDcpsEmpId, Long lLngYearId, String lStrType, int monthId,String Fromdate,String Todate)
	{
		...
		//	lSBQuery.append(" WHERE trn.dcps_emp_id =:dcpsEmpId AND trn.voucher_date between '" + Fromdate+ "' AND '" + Todate + "'   ");
	...
	public Double getTotalMissingAmountDetails(Long lLngDcpsEmpId, Long lLngYearId,String fromDate,String toDate)
	{
		...
		//	lSBQuery.append(" SELECT nvl(sum(CONTRIBUTION),0) FROM  TRN_DCPS_CONTRIBUTION where REG_STATUS=1 and MISSING_CREDIT_APPROVAL_DATE between '"+fromDate+"' and '"+toDate+"'  and EMPLOYER_CONTRI_FLAG='Y'  and DCPS_EMP_ID=:dcpsEmpId  and (is_missing_credit='Y' or IS_CHALLAN='Y')  ");
			...
			lSBQuery.append(" (  ");
			lSBQuery.append(" SELECT nvl(sum(CONTRIBUTION),0) as contri FROM  TRN_DCPS_CONTRIBUTION where REG_STATUS=1 and startdate between '2005-04-01' and '2015-03-31' ");
			...
			lSBQuery.append(" SELECT nvl(sum(CONTRIBUTION),0)  as contri FROM TRN_DCPS_CONTRIBUTION WHERE REG_STATUS=1 AND VOUCHER_DATE BETWEEN '"+fromDate+"' and '"+toDate+"'   and EMPLOYER_CONTRI_FLAG='Y' and startdate between '2005-04-01' and '2015-03-31' and DCPS_EMP_ID=:dcpsEmpId  and ");
		//	lSBQuery.append(" and STARTDATE between '2005-04-01' and '2015-03-31' ");
	...
	//jitu
	public String getEmpDCPSEMPID(String lStrSevaarthId)
	{
		List lLstData = null;
		String IsDCPSEMPID = "";
		/*SessionFactory sessionFactory1=new Configuration().configure("hibernate.cfg.xml").buildSessionFactory();
		...
		try{
			StringBuilder lSBQuery = new StringBuilder();
			lSBQuery.append("SELECT DCPS_EMP_ID FROM MST_DCPS_EMP WHERE SEVARTH_ID = :lStrSevaarthId");
			ghibSession = gObjSessionFactory.getCurrentSession();
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
			lQuery.setString("lStrSevaarthId", lStrSevaarthId);
			lLstData = lQuery.list();

			if(lLstData.size() > 0){
				IsDCPSEMPID = lLstData.get(0).toString();
			}
		}catch(Exception e){
			gLogger.error("Exception in getDdoName:" + e, e);
		}finally{
	...
	{
		List lLstData = null;
		String isDDOCODE = null;
		String isDEPDDOCODE = null;
		/*SessionFactory sessionFactory1=new Configuration().configure("hibernate.cfg.xml").buildSessionFactory();
        Session neardrSession = sessionFactory1.openSession();*/
		try{
			StringBuilder lSBQuery = new StringBuilder();
			lSBQuery.append("SELECT DDO_CODE FROM MST_DCPS_EMP WHERE SEVARTH_ID = :lStrSevaarthId");
			ghibSession = gObjSessionFactory.getCurrentSession();
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
			lQuery.setString("lStrSevaarthId", lStrSevaarthId);
			lLstData = lQuery.list();
			...
			if(lLstData.size() > 0){
				isDDOCODE = lLstData.get(0).toString();
	...
	public String getEmployeeDEPDDOCODE(String lStrSevaarthId)
	{
		List lLstData = null;
		String isDDOCODE = null;
		String isDEPDDOCODE = null;
		...
		try{
			StringBuilder lSBQuery = new StringBuilder();
			lSBQuery.append("SELECT DEPT_DDO_CODE FROM MST_DCPS_EMP WHERE SEVARTH_ID = :lStrSevaarthId");
			ghibSession = gObjSessionFactory.getCurrentSession();
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
			lQuery.setString("lStrSevaarthId", lStrSevaarthId);
			lLstData = lQuery.list();
