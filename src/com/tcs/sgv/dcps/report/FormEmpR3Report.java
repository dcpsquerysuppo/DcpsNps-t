...
public class FormEmpR3Report extends DefaultReportDataFinder implements ReportDataFinder
{
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria)throws ReportException
	{
		...
		try{
			...
			if (lObjReport.getReportCode().equals("8000095")) 
			{
				...
				//Long lLngTreasuryId = Long.valueOf((String)lObjReport.getParameterValue("treasuryCode"));
				...
				SBQueryy.append(" SELECT distinct emp.DCPS_ID,substr(emp.ddo_code,1,4) as CUR_TREASURY_CD,emp.ddo_code as CUR_DDO_CD,emp.EMP_NAME,emp.DOB,emp.DOJ,emp.PAY_COMMISSION,emp.DCPS_EMP_ID  ");
				SBQueryy.append(" FROM mst_dcps_emp emp inner join MST_DCPS_DDO_OFFICE off on emp.CURR_OFF=off.DCPS_DDO_OFFICE_MST_ID ");
				SBQueryy.append(" inner join TRN_DCPS_CONTRIBUTION trn on trn.DCPS_EMP_ID=emp.DCPS_EMP_ID ");
				SBQueryy.append("  left outer join MST_DCPS_CONTRIBUTION_YEARLY yr on yr.DCPS_ID=emp.DCPS_ID inner join SGVC_FIN_YEAR_MST fin on fin.FIN_YEAR_ID=trn.FIN_YEAR_ID ");
				SBQueryy.append(" where emp.SEVARTH_ID ='"+UserName.substring(0, 11)+"' and fin.FIN_YEAR_ID="+lLngYearId+" ");  
				StrSqlQuery = SBQueryy.toString();
				rs3 = smt.executeQuery(StrSqlQuery);
				if (rs3.next()) 
				{
					 treasuryCode=rs3.getString("CUR_TREASURY_CD");
					 ddoCode=rs3.getString("CUR_DDO_CD");
				...
				String lStrTreasuryName = getLocationName(Long.parseLong(treasuryCode));
				String lStrYear = lObjDcpsCommonDAO.getFinYearForYearId(lLngYearId);
				String lStrDdoName = getDdoName(ddoCode);
				String lStrEmpName = getEmpName(Long.valueOf(Long.parseLong(dcpsEmpId)));
				...
				rowList1.add(new StyledData("Birth Date : "
						...
						+ "Treasury : (" + treasuryCode.toString() + ") "
						+ lStrTreasuryName ,
				...
				rowList1 = new ArrayList();
				rowList1.add(new StyledData("DDO :("+ddoCode+")"+ lStrDdoName, boldVO));
				dataList.add(rowList1);
				...
				lLstOpeningBalcTier1 = getOpeningBalanceTier1(Long.valueOf(Long.parseLong(dcpsEmpId)), lLngYearId);
				...
				int count = 0;
				lDblRegRecvryEmp = getEmpAllDtlsTire1(Long.parseLong(dcpsEmpId), lLngYearId, "700046",fromDate,toDate);
				...
				lDblDelyRecvryEmp = getEmpAllDtlsTire1(Long.parseLong(dcpsEmpId), lLngYearId, "700047",fromDate,toDate);
				for(int monID=3;monID<=14;monID++)
				{
					int newIndex = monID;
					if(monID > 12)
				...
				lDblDAArrearEmp = getEmpAllDtlsTire1(Long.parseLong(dcpsEmpId), lLngYearId, "700048",fromDate,toDate);
				for(int monID=3;monID<=14;monID++)
				{
					int newIndex = monID;
					if(monID > 12)
				...
				lDblPayArrearEmp = getEmpAllDtlsTire1(Long.parseLong(dcpsEmpId), lLngYearId, "700049",fromDate,toDate);
				for(int monID=3;monID<=14;monID++)
				{
					int newIndex = monID;
					if(monID > 12)
			...
			lDbMissingCreditTier1 = getEmpAllMissingCreditTire1(Long.parseLong(dcpsEmpId), fromDate,toDate,lLngYearId);
			 empMisIntTier=getTier1IntMissingDetails(Long.parseLong(dcpsEmpId), lLngYearId);
			for(int monID=3;monID<=14;monID++)
			{
				int newIndex = monID;
				if(monID > 12)
				...
				/*	missingMon = (obj[0] != null) ? obj[0].toString() : "";
					...
					if(newIndex == Integer.parseInt(obj[3].toString())){
						...
						rowList.add(" ");
					/*	if(newIndex>3){
						rowList.add(new StyledData(missingMonth + "-" + arrYearSplit[0],boldVO));
						}
						else
						{
            ...
            lTotalAmt=getTotalAmountDetails(Long.parseLong(dcpsEmpId), lLngYearId, fromDate, toDate);
            lTotalMissingamt=getTotalMissingAmountDetails(Long.parseLong(dcpsEmpId), lLngYearId, fromDate, toDate);
			...
			lLstDataList.add(rowList);
			  ...
			  rowList = new ArrayList();
				...
				Double empTier = getTier1TotalDetails(Long.parseLong(dcpsEmpId), lLngYearId);
			 	lTotalAmt=getTotalAmountDetails(Long.parseLong(dcpsEmpId), lLngYearId, fromDate, toDate);

				/*if(empTier != null )
				{
					lObj = (Object[]) lLstEmpTier1.get(0);
					if(lObj[0] != null){
						lLngTotalEmpT1 = Math.round(Double.parseDouble(lObj[0].toString()));
					}
					if(lObj[1] != null){
						lLngTotalEmplyrT1 = Math.round(Double.parseDouble(lObj[1].toString()));
					}
					if(lObj[2] != null){
						lLngTotalIntrstT1 = Math.round(Double.parseDouble(lObj[2].toString()));
					}				
				}		*/
				...
				/* lLngEmpT1 = lTotalAmt;
						lLngEmplyrT1 = Math.round(Double.parseDouble(lObj[0].toString()));*/


				/*if(lObj[1] != null){
						lLngEmplyrT1 = Math.round(Double.parseDouble(lObj[1].toString()));
					}*/
				...
				// Tier 1 Employee total formula changed
				/*lDoubleTotalEmpT1 = lDblRegRecvryEmp + lDblDelyRecvryEmp + lDblDAArrearEmp + lDblPayArrearEmp;
				lLngTotalEmpT1 = lDoubleTotalEmpT1.longValue();*/

				rowList.add("");

				// Tier 2 Employer total formula changed
				/*lDoubleTotalEmplyrT1 = lDblRegRecvryEmplyr + lDblDelyRecvryEmplyr + lDblDAArrearEmplyr + lDblPayArrearEmplyr;
				lLngTotalEmplyrT1 = lDoubleTotalEmplyrT1.longValue();*/
				...
				String [] arr1=null;
				String test=null;
				lLstDtlsTier2 = getTier2Details(Long.parseLong(dcpsEmpId), lLngYearId);
				lLstDtlsFinalTier1=getTier1FinalDetails(Long.parseLong(dcpsEmpId), lLngYearId);
				lLstDtlsFinalMissingTier1=getTier1FinalIntMissingDetails(Long.parseLong(dcpsEmpId), lLngYearId);
				...
				if(lLstDtlsTier2 != null && lLstDtlsTier2.size() > 0)
				{
					...
					if(lObj[1] != null){
						...
						lLngYearlyContriTier2 = Math.round(getYearlyArrAmountOfEmp(Long.parseLong(dcpsEmpId),lLngYearId));
					}
					if(lObj[2] != null){
					...
					/*gLogger.info("emplrIntrstT2 "+emplrIntrstT2);
					 arr=emplrIntrstT2.split("\\.");
					 arr1=empIntrstT2.split("\\.");*/
				}
	...
	public List getYear(String lStrLangId, String lStrLocId) {
		...
		try {
			...
			lsb = new StringBuffer(
					...
					+ lStrLangId
					+ "'  and fin_year_code between '2012' and '2022' order by fin_year_code ");
	...
	public List getEmpAllDtlsTire1(Long lLngDcpsEmpId, Long lLngYearId, String lStrType,String fromDate,String toDate)
	{
		...
		try{
			StringBuilder lSBQuery = new StringBuilder();
		/*	lSBQuery.append("  SELECT substr(upper(mst.MONTH_NAME),1,3)||'-'||case when mst.month_id >4 then cast(fin.FIN_YEAR_CODE as bigint) else cast((fin.FIN_YEAR_CODE)+1 as bigint) end as year,fin.FIN_YEAR_DESC,sum(trn.contribution),mst.month_id   from TRN_DCPS_CONTRIBUTION trn    ");
			if (lStrType.equals("700047"))
			{
				gLogger.info("lStrType is"+lStrType);
			...
			lSBQuery.append("  WHERE  trn.fin_year_id="+lLngYearId+" and trn.DCPS_EMP_ID ="+lLngDcpsEmpId+"  and  trn.TYPE_OF_PAYMENT="+lStrType+" ");
			lSBQuery.append("  AND trn.REG_STATUS = 1  and  trn.EMPLOYER_CONTRI_FLAG = 'Y'  and trn.is_missing_credit is null and month(trn.STARTDATE) <> 3 ");
			...
			/*	
				for (Object object : lLstData) {

					Object[]obj = (Object[]) object;

					String month = (obj[0]!=null) ? obj[0].toString() : "";
					gLogger.info("month  is------------"+month );

					String Amount = (obj[1]!=null) ? obj[1].toString() : "";
					gLogger.info("Amount  is------------"+Amount );		
					lDblRecovery.add(month+"--"+Amount.toString());
	...
	/*public List getEmployerAllDtlsTire1(Long lLngDcpsEmpId, Long lLngYearId, String lStrType, int monthId,String Fromdate,String Todate)
	{
		//List lLstData = null;
		List lDblRecovery = new ArrayList();
		try{
...
*/
	public Double getTier1TotalDetails(Long lLngDcpsEmpId, Long lLngYearId)
	{
		...
		try{
			StringBuilder lSBQuery = new StringBuilder();
			/*lSBQuery.append("SELECT OPEN_EMPLOYEE+CONTRIB_EMPLOYEE,OPEN_EMPLOYER+CONTRIB_EMPLOYER,INT_CONTRB_EMPLOYEE + INT_CONTRB_EMPLOYER ");
			lSBQuery.append("FROM MST_DCPS_CONTRIBUTION_YEARLY where dcps_id = (select dcps_id from mst_dcps_emp where dcps_Emp_id = :dcpsEmpId ) ");			
			lSBQuery.append("AND YEAR_ID = :year ");
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
			lQuery.setLong("dcpsEmpId", lLngDcpsEmpId);
			lQuery.setLong("year", lLngYearId);*/
