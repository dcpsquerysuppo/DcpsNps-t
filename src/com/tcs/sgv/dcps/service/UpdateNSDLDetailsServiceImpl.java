...
public class UpdateNSDLDetailsServiceImpl extends ServiceImpl  {
	...
	public ResultObject loadNPSNSDLUpdate(Map inputMap) {
		...
		try {
			...
			if((StringUtility.getParameter("fileIds", request)!=null)&&(StringUtility.getParameter("fileIds", request)!="")){
				...
				for (Integer lInt = 0; lInt < fileids.length; lInt++)
				{
					...
					lSBStatus=lObjNsdlDAO.updateTransactionDetails(fileArray[lInt],tranArr[lInt]);
			...
			resObj.setResultValue(inputMap);
				...
				resObj.setResultValue(inputMap);
				resObj.setViewName("UpdateNPSVALIDATE");
			}
	...
	public ResultObject loadNPSNSDLUpdateRATF(Map inputMap) {
		...
		try {
			...
			if((StringUtility.getParameter("fileIds", request)!=null)&&(StringUtility.getParameter("fileIds", request)!="")){
				...
				{
					//lObjNsdlDAO.updateTransactionDetails(fileArray[lInt],tranArr[lInt],year,month);
					lObjNsdlDAO.updateTransactionDetailsRATF(fileArray[lInt],tranArr[lInt]);
			...
			List nsdlDeatils=lObjNsdlDAO.getAllDataRATF(currentyear.toString(), currentmonth.toString(), trCode);
			//$t 2020 17-1 added 2020 year
			List lLstYears = lObjNsdlDAO.getFinyear();
			DcpsCommonDAO lObjDcpsCommonDAO = new DcpsCommonDAOImpl(null, serv
					.getSessionFactory());
			...
			resObj.setResultValue(inputMap);
			resObj.setViewName("RATFUpdateNPSVALIDATE");
...
////$t15Jun2023
	public ResultObject chkTranAlreadyMapped(Map<String, Object> inputMap) {
		ServiceLocator serv = (ServiceLocator) inputMap.get("serviceLocator");
		HttpServletRequest request = (HttpServletRequest) inputMap.get("requestObj");
		ResultObject resObj = new ResultObject(ErrorConstants.SUCCESS);
		UpdateNSDLDetailsDAOImpl updateNSDLDAO = new UpdateNSDLDetailsDAOImpl(null, serv.getSessionFactory());
		setSessionInfo(inputMap);
		try {
			String status = "N";
			String searchTransaction = StringUtility.getParameter("searchTransaction", request);
			
			int chkCount;
			if ((searchTransaction.length() > 0 || searchTransaction.length() > 0)) {
				chkCount=updateNSDLDAO.checkTranAlreadyMapped(searchTransaction);
			if(chkCount>0)
				status = "Y";
			}
