	...
	public Double InterestCalculation( Double Inst1,Double inst2,Double inst3,Double inst4,Double inst5, String Type ) throws Exception{
		...
		for (int i=0;i<lstLoopingForTierII.size();i++)
		{
			...
			if(i>4)
			{
		       ...
		       if(tupleSub[0].toString().equals("36") && Type.equals("Deputation")){			
...
//					resObj.setViewName("TierIIOrderReport");
	...
	public ResultObject updateFiveInstAmt(Map<String, Object> inputMap) {
		...
		try {
			...
			String cmbInstDtlsI = "NA";
			String cmbInstDtlsII = "NA";
			String cmbInstDtlsIII = "NA";
			String cmbInstDtlsIV = "NA";
			String cmbInstDtlsV = "NA";
	...
	//get namunaF
		...
		//forward Beams Code
		 public ResultObject forwardBillDataToBEAMSForNPSTierIIDept(Map objectArgs) {
			 gLogger.info("inside forwardBillDataToBEAMSForNPS");
			 HttpServletRequest request = (HttpServletRequest) objectArgs.get("requestObj");
				ServiceLocator serv = (ServiceLocator) objectArgs.get("serviceLocator");
				ResultObject resultObject = new ResultObject(0);
...
//				String DDOCode="";
				...
				if (resultMap != null && !resultMap.isEmpty()) {
					...
					if ("00".equals(statusCode)) {
						flag = "Y";
						lObjNsdlDAO.updateTierIIBillDetailsDept(authNo,paybillId,flag);
			...
			public ResultObject loadTierIIRegularReport(Map inputMap) {

				ResultObject resObj = new ResultObject(0, "FAIL");
				String lStrTempFromDate = null;
				try {
					setSessionInfo(inputMap);
					TierIISixPcArrearDaoImpl lObjNsdlDAO = new TierIISixPcArrearDaoImpl(null,this.serv.getSessionFactory());
					String typeBill = StringUtility.getParameter("Flag", request);
					inputMap.put("typeBillFlag", typeBill);
					resObj.setResultValue(inputMap);
					resObj.setViewName("TierIISummaryReport");
				} catch (Exception e) {
					e.printStackTrace();
