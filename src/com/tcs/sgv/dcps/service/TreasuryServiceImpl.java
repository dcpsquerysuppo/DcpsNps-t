	...
	*//**
	...
	public ResultObject loadEmpDeputation(Map inputMap) {
		...
		try {
			...
			String lStrWithOrWithoutEmplrContri = StringUtility.getParameter("Use", request).trim() ;//WithoutEmplrContri
			...
			lObjTreasuryDAO = new TreasuryDAOImpl(TreasuryServiceImpl.class,serv.getSessionFactory());
			...
			String lStrUserType = lObjGpfLnaDashboardDao.getUserType(this.gLngUserId);//lStrUserType=Treasury
		    this.gLogger.info("lStrUserType" + lStrUserType);
		    ////$t IRRI
		    ////$t 13-9-20 IRRI
			if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {
				gStrLocationCode = ((TreasuryDAOImpl) lObjTreasuryDAO).getTreasuryCodeForAst(gStrLocationCode).toString();
...
//Added by ashish to get employee list of acc main by IAS,IPS and IFS
	...
	public ResultObject showEmpDeputationList(Map inputMap) {
		...
		String loadDCPS="";
		try {
			setSessionInfo(inputMap);//ifms.htm?actionFlag=loadEmpDeputation&queryString=Attach&Use=WithoutEmplrContri&elementId=1900001803
			lStrQueryString = StringUtility.getParameter("queryString", request).trim();//Attach
			loadDCPS = StringUtility.getParameter("loadDCPS", request).trim();
			gLogger.info("############loadDCPS"+loadDCPS);
		    String lStrWithOrWithoutEmplrContri = StringUtility.getParameter("Use", request).trim() ;
			inputMap.put("WithOrWithoutEmplrContri", lStrWithOrWithoutEmplrContri);
			lObjTreasuryDAO = new TreasuryDAOImpl(TreasuryServiceImpl.class,serv.getSessionFactory());
			lLstReasons = IFMSCommonServiceImpl.getLookupValues("Reason For Deslection", gLngLangId, inputMap);//gLngLangId=1
			//lLstOffice = lObjTreasuryDAO.getDummyOffices(gStrLocationCode,lStrWithOrWithoutEmplrContri);//gStrLocationCode=1111
		    ////$t 13-9-20 IRRI
		    GpfLnaDashboardDao lObjGpfLnaDashboardDao = new GpfLnaDashboardDaoImpl(null, this.serv.getSessionFactory());
		    String lStrUserType = lObjGpfLnaDashboardDao.getUserType(this.gLngUserId);//lStrUserType=Treasury
			this.gLogger.info("lStrUserType" + lStrUserType);
			...
			{
				ddoCode=lObjTreasuryDAO.getEmpDDOCode(lStrSevarthId, lStrEmpName, Long.valueOf(gStrLocationCode), flag);//3707001669
				if(ddoCode!=null && !"".equalsIgnoreCase(ddoCode))
				{
					//isDdoCodeNull="false";3707001669
				...
				returnFlag=lObjTreasuryDAO.chkIfDdoOfHodDefined(lStrSevarthId, lStrEmpName);//returnFlag=true
				if(returnFlag)
				{
					hodFlag="true";
				}
			...
			gLogger.info("############deptname"+deptname);//isBelongToTreasury=false,hodFlag=true,deptname=dept,officeName=office,ddoCode=3707001669,loadDCPS=no
			String lSBStatus = getResponseXMLDoc(isBelongToTreasury,hodFlag,deptname,officeName,ddoCode,loadDCPS).toString();
			gLogger.info("############"+lSBStatus);//<SEVAARTH>true</SEVAARTH><HOD>true</HOD><DEPT>Dept</DEPT><OFFICENAME>office</OFFICENAME><OFFICE>false</OFFICE><DCPSFLAG>No</DCPSFLAG></XMLDOC>
	...
	public ResultObject loadChallanForDummy(Map inputMap) throws Exception {
		...
		String Irrigation="no";
		////$t
		this.gLogger.info("before try*****");
		try {
			setSessionInfo(inputMap);
			...
			String lStrUseType = StringUtility.getParameter("Use", request).trim();//lStrUseType=WithEmplrContri
			
//		    ////$t 20-04-2020
//			String Irrigation = StringUtility.getParameter("Irrigation", request).trim();
...
//			////$t
			...
			{
				isNPS = StringUtility.getParameter("isNPS", request).trim() ;//Ask? where we setParameter()
			}
			inputMap.put("WithOrWithoutEmplrContri", lStrWithOrWithoutEmplrContri);
			...
			if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {
			 ...
			 if (!StringUtility.getParameter("dummyOfficeId", request).trim().equals("") && StringUtility.getParameter("dummyOfficeId", request) != null
				 ...
				 this.gLogger.info("after when you select *****");
			    ...
			    ////$t 20-04-2020 IRRI for To login show approved and reject button and t show only for amravati 6101
				//if(((lStrDummyOfficeId.equals("X00024")||lStrDummyOfficeId.equals("X00025")||lStrDummyOfficeId.equals("X00004"))&&(gStrLocationCode.equals("6101")||gStrLocationCode.equals("1111")))){
				if((((lStrDummyOfficeId.equals("X00013"))||(lStrDummyOfficeId.equals("X00014"))||(lStrDummyOfficeId.equals("X00017")))&&(gStrLocationCode.equals("6101")||gStrLocationCode.equals("1111")))){//X00025('X00004','X00007')
					Irrigation="yes";
				}
				////$t
				...
				//chk? for every login(DDO,DDO_Ast,To) employee offices returns same
				if(isNPS!=null && !"".equalsIgnoreCase(isNPS) && "yes".equalsIgnoreCase(isNPS))
				{
					if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {//here$t
						...
						lListEmployeesFromDummyOffice = lObjTreasuryDAO.getEmployeesFromDummyOfficeNPS(lStrDummyOfficeId,monthId,yearId,gStrLocationCode,lStrSubmittedOrNot,lStrWithOrWithoutEmplrContri,Irrigation);
				...
				else
				{
					...
					if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {//$there
						...
						lListEmployeesFromDummyOffice = lObjTreasuryDAO.getEmployeesFromDummyOffice(lStrDummyOfficeId,monthId,yearId,gStrLocationCode,lStrSubmittedOrNot,lStrWithOrWithoutEmplrContri,Irrigation);////$t irri
					}
				...
				this.gLogger.info("lListEmployeesFromDummyOffice try*****");
				if(lListEmployeesFromDummyOffice!=null && lListEmployeesFromDummyOffice.size()>0)
				{
					...
					for (int liCtr = 0; liCtr < lListEmployeesFromDummyOffice.size(); liCtr++) {
						...
						////$t 20-4-2020 
						if (lStrUserType.equals("DDO") || lStrUserType.equals("Treasury") ||lStrUserType.equals("DDO Assistant") || lStrUserType.equals("EmployeeSelfService")) {//$there
							if(obj[20]==null){
								statusFlag="none";
							}else{
						        statusFlag=obj[20].toString();
							}
						}
						//$t
				...
				//$$$$ I $$$$ In hibernate table Cmn_Lookup_Mst includes CMN_LANGUAGE_MST table 
				listTypeOfPayment = IFMSCommonServiceImpl.getLookupValues("TypeOfPaymentDCPS", 1L,inputMap);//Regular,Pay Arrear,Delayed etc
				...
				List<CmnLookupMst> listPayCommissionOld = IFMSCommonServiceImpl
						.getLookupValues("PayCommissionDCPS", SessionHelper
								.getLangId(inputMap), inputMap);//6th Pay Commission,Padmanabhan Comm. etc
				...
				for (CmnLookupMst lObjCommonLookupMst : listPayCommissionOld) {
					...
					if (Long.valueOf(lObjCommonLookupMst.getLookupId()).toString().equals(
						...
						//this.gLogger.info("listPayCommission *****"+listPayCommission.get(0));
					}
					if (Long.valueOf(lObjCommonLookupMst.getLookupId()).toString().equals(
							"700016")) {
						tempCmnLookupMst = new CmnLookupMst();
			...
			//this.gLogger.info("after not select*****");
			...
			////$t 20-04-2020 
			inputMap.put("loginUserType", lStrUserType);
			inputMap.put("statusFlag", statusFlag);
			inputMap.put("Irrigation", Irrigation);
			////$t
	...
	public ResultObject saveContriForDeptn(Map inputMap) {
		...
		try {
			...
			TrnDcpsDeputationContribution[] lArrTrnDcpsDeputationContribution = (TrnDcpsDeputationContribution[]) inputMap.get("lArrTrnDcpsDeputationContribution");//ask?

			TreasuryDAO lObjTreasuryDAO = new TreasuryDAOImpl(TreasuryServiceImpl.class, serv.getSessionFactory());
			
			lStrUse = StringUtility.getParameter("Use", request).trim();
			isNPS=StringUtility.getParameter("isNPS", request).trim();
			
			for (Integer lInt = 0; lInt < lArrTrnDcpsDeputationContribution.length; lInt++) {
				//gLogger.info("lArrTrnDcpsDeputationContribution.length*****"+lArrTrnDcpsDeputationContribution.length);//lArrTrnDcpsDeputationContribution.length=total emp selected
				lBlFlag = false;
				IdGenerator id=new IdGenerator();//chk?
				//gLogger.info("inputMap*****"+inputMap);//hash value
				
				lLongDeptnContriId=id.PKGenerator("TRN_DCPS_DEPUTATION_CONTRIBUTION", inputMap);//chk?
				//lLongDeptnContriId=(long)9945012;
				///$t 5-9-2020
				///oct 9916027; sept9916026 aug9916022 july 9916020;//June9916012;
				//$$$$ I $$$$ pass parameter to PKGenerator() method present in jar and returns  lLongDeptnContriId.
				...
				lObjTreasuryDAO.create(lArrTrnDcpsDeputationContribution[lInt]);//ask?
				
				lBlFlag = true;
				if (lBlFlag) {
					inputMap.put("currentDeptnCntrbtnVO",lArrTrnDcpsDeputationContribution[lInt]);
 					//gLogger.info("lArrTrnDcpsDeputationContribution[lInt]*****"+lArrTrnDcpsDeputationContribution[lInt]);//hash0
					lLongVoucherNo = Long.valueOf(lArrTrnDcpsDeputationContribution[lInt].getChallanNo());//when u click btnSubmit then in class TrnDcpsDeputationContribution of setChallanNo() will be set  
					...
					resObj = serv.executeService("insertCntrbtnForDeptn",inputMap);//insertCntrbtnForDeptn() called 
				}
			}
	...
	public ResultObject insertCntrbtnForDeptn(Map objectArgs) {
		...
		TrnDcpsDeputationContribution lObjTrnDcpsDeputationContribution = (TrnDcpsDeputationContribution) objectArgs.get("currentDeptnCntrbtnVO");
		//in this saveContriForDeptn() we put currentDeptnCntrbtnVO
		...
		try {
		    ...
		    this.gLogger.info("lStrUserType" + lStrUserType);
			////$t
		    this.gLogger.info("lObjTrnDcpsDeputationContribution.getDcpsEmpId()-->" + lObjTrnDcpsDeputationContribution.getDcpsEmpId());
		    this.gLogger.info("Long.valueOf(lObjTrnDcpsDeputationContribution.getDcpsEmpId()).getDcpsEmpId()-->" + Long.valueOf(lObjTrnDcpsDeputationContribution.getDcpsEmpId()));
		...
		/////$t 20-04-2020 IRRI
			...
			if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {//$there
			lLongTreasuryCode = Long.parseLong(((TreasuryDAOImpl) lObjTreasuryDAO).getTreasuryCodeForAst(lLngLocId.toString()));
			}else{
			lLongTreasuryCode =lLngLocId;	
			}
			////$t            
			...
			lStrSchemeCode = objectArgs.get("lLongVoucherNo").toString().trim();
			...
			{
				lLngDcpsContriVoucherId = IFMSCommonServiceImpl.getNextSeqNum("mst_dcps_contri_voucher_dtls", objectArgs);//991000070052
				//$$$$ I $$$$ objectArgs pass as parameter and table name then we get lLngDcpsContriVoucherId
			...
			//	lObjMstDcpsContriVoucherDtls.setTreasuryCode(Long.valueOf(lObjTrnDcpsDeputationContribution.getTreasury()));
				...
				//$$$$ I $$$$ on a particular table after set()
				//execute this method lObjTreasuryDAO.create(lObjMstDcpsContriVoucherDtls);
		...
		*/
		...
		/////$t 20-04-2020 IRRI
			        ...
			        if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {//$there
						lObjTrnDcpsContribution.setLocId(lLongTreasuryCode);
					}else{
			...
			if(isNPS!=null && !"".equalsIgnoreCase(isNPS) && "yes".equalsIgnoreCase(isNPS))
			{
				/////$t 20-04-2020 IRRI doubt
				if (lStrUserType.equals("EmployeeSelfService")||lStrUserType.equals("DDO Assistant")) {//$there////$t irri 13-1-2021 change status 'M' to status 'D'
				lObjTrnDcpsContribution.setStatus('D');
				}else if(lStrUserType.equals("DDO")){
				lObjTrnDcpsContribution.setStatus('N');
				}else{
...
/*	////$t 20-04-2020 IRRI
	public ResultObject updateContriForDeptnDDO(Map inputMap) {
		...
		try {
			...
			txtStartDates = StringUtility.getParameter("txtStartDate", request).trim();
			String txtStartDate[] = txtStartDates.split("~");
			//System.out.println("txtStartDates-->" + txtStartDate);
			
			txtEndDates = StringUtility.getParameter("txtEndDate", request).trim();
			String txtEndDate[] = txtEndDates.split("~");
			//System.out.println("txtEndDates-->" + txtEndDate);
			
			if (dcpsEmpID != null && dcpsEmpID.length > 0)
			{
				for (int i = 0; i < dcpsEmpID.length; i++) {
					lObjTreasuryDAO.updateContriFlagDDO(dcpsEmpID[i],monthId,yearId,txtStartDate[i],txtEndDate[i]);
					lBlFlag=true;
				}
			}	
	...
	////$t
////$t 20-04-2020 IRRI
	public ResultObject updateContriForDeptnTO(Map inputMap) {
		...
		try {
			...
			{
				for (int i = 0; i < dcpsEmpID.length; i++) {
					lObjTreasuryDAO.updateContriFlagTO(dcpsEmpID[i],monthId,yearId,txtStartDate[i],txtEndDate[i],isNPS,lStrUse);
					lBlFlag=true;
				}
			}	
...
////$t 20-04-2020
	public ResultObject rejectContriForDeptnDDO(Map inputMap) {
		...
		try {
			...
			{
				for (int i = 0; i < dcpsEmpID.length; i++) {
					lObjTreasuryDAO.rejectContriFlagDDO(dcpsEmpID[i],monthId,yearId,txtStartDate[i],txtEndDate[i]);
					lBlFlag=true;
				}
			}	
...
////$t 20-04-2020 IRRI
	public ResultObject rejectContriForDeptnTO(Map inputMap) {
		...
		try {
			...
			{
				for (int i = 0; i < dcpsEmpID.length; i++) {
					lObjTreasuryDAO.rejectContriFlagTO(dcpsEmpID[i],monthId,yearId,txtStartDate[i],txtEndDate[i]);
					lBlFlag=true;
				}
			}	
...
////$t IRRI irri 13-1-2021
	public ResultObject updateContriForDeptnAsDraft(Map inputMap) {
		...
		try {
			...
			{
				for (int i = 0; i < dcpsEmpID.length; i++) {
					lObjTreasuryDAO.updateContriFlagDdoAsst(dcpsEmpID[i],monthId,yearId,txtStartDate[i],txtEndDate[i]);
					lBlFlag=true;
				}
			}	
