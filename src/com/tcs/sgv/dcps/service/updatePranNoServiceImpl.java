...
public class updatePranNoServiceImpl extends ServiceImpl {
	...
	////$t12Jun2023
	public ResultObject chkPranAlreadyAttach(Map<String, Object> inputMap) {
		ServiceLocator serv = (ServiceLocator) inputMap.get("serviceLocator");
		HttpServletRequest request = (HttpServletRequest) inputMap.get("requestObj");
		ResultObject resObj = new ResultObject(ErrorConstants.SUCCESS);
		updatePranNoDAOImpl updatePranNoDAO = new updatePranNoDAOImpl(null, serv.getSessionFactory());
		setSessionInfo(inputMap);
		try {
			String status = "N";
			String searchPran = StringUtility.getParameter("searchPran", request);
			
			int chkCount;
			if ((searchPran.length() > 0 || searchPran.length() > 0)) {
				chkCount=updatePranNoDAO.checkPranExistInAttach(searchPran);
			if(chkCount>0)
				status = "Y";
			}
	...
	private StringBuilder getResponseUpdatedTO(String flag) {
		logger.info("Flag is  in AJAX********" + flag);
