...
public class GPFGeneratePayBillReport extends DefaultReportDataFinder implements ReportDataFinder 
{
	...
	///
	private Long gStrPostId = null; /* STRING POST ID */
	...
	public Collection findReportData(ReportVO report, Object criteria) throws ReportException 
	{
		...
		gStrPostId = lObjLoginVO.getLoggedInPost().getPostId(); //added by brijoy 28 - 02- 2019
		gLogger.info("hiii the location Code is ******&&&&&&&#########******"+gStrLocCode);
		gLogger.info("hiii the getPostId Code is ******&&&&&&&#########******"+gStrPostId);
		...
		try{
			...
			if(report.getReportCode().equals("800010"))
			{
				...
				gLogger.error("lStrGpfAccNo "+lStrGpfAccNo );
				...
				gLogger.error("******lStrSanctionedAmount***********"+lStrSanctionedAmount);
				...
				String[] parts = lStrOrderNumber.split("/");
				String part1 = parts[0]; 
				String strDdoCode = parts[1]; 
				String part3 = parts[2]; 
				String part4 = parts[3]; 
				gLogger.error("********part1*********"+part1);
				gLogger.error("********part2*********"+strDdoCode);
				gLogger.error("********part3*********"+part3);
				gLogger.error("********part4*********"+part4);
				
				List lstTrsry = getTreasuryNameandCodeOfEmp(strDdoCode);
				String strTrsryName = "";
				String strTrsryCode = "";
				//Added By vivek sharma 07 june 2017
				if(lstTrsry != null && !lstTrsry.isEmpty()){
					Object[] lobj = (Object[])lstTrsry.get(0);

					if(lobj[0] != null){
						strTrsryName = lobj[0].toString();
					}
					if(lobj[1] != null){
						strTrsryCode = lobj[1].toString();
					}
				}
			
				gLogger.error("********strTrsryName*********"+strTrsryName);
				gLogger.error("********strTrsryCode*********"+strTrsryCode);
				
				List lStrEmpDetails = lObjGPFRequestProcess.getNameAndDesignation(lStrSevaarthId);
				gLogger.error("********lStrEmpDetails*********"+lStrEmpDetails);
				gLogger.error("********lStrEmpDetailsIze*********"+lStrEmpDetails.size());
				...
				gLogger.info("lStrNameOfOffice****"+lStrNameOfOffice);
                 ...
                 //Added By vivek Sharma 04 July 2017
				String lStrEmployerDsgnName = lObjGPFRequestProcess.getEmployerDsgnName(lLngPostId);
				gLogger.info("lStrEmployerDsgnName****"+lStrEmployerDsgnName);
				
				String emplyeeOfficerName = lObjGPFRequestProcess.getEmployerNameFrmPostId(lLngPostId.toString());
				gLogger.info("emplyeeOfficerName****"+emplyeeOfficerName);
				
				String lStrEmployerOfficeName = lObjGPFRequestProcess.getEmployerOfficeName(strDdoCode);
				...
				if(lLstEmpDetails != null && lLstEmpDetails.size() > 0){
					...
					if(lstOrderNo != null && lstOrderNo.size() > 0){
						...
						if(lstOrderNo.get(0) != null ){
							...
							if(obj2[2] != null)
								strOrderMnth = obj2[2].toString();
						}
					}
					gLogger.info("lStrOrderNo****"+lStrOrderNo);
					gLogger.info("bilNo****"+bilNo);
					gLogger.info("strOrderMnth****"+strOrderMnth);
				...
				if(lStrreqType.equalsIgnoreCase("RA")){
					...
					rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENHEADERRA"),centerAlign));
					dataList.add(rowList);
				}else if(lStrreqType.equalsIgnoreCase("NRA")){
					rowList = new ArrayList<Object>();
					rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENHEADER1NRA"),centerAlign));
					dataList.add(rowList);
				}else if(lStrreqType.equalsIgnoreCase("CS")){
					rowList = new ArrayList<Object>();
					rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENHEADER1CONV"),centerAlign));
					dataList.add(rowList);
				}else if(lStrreqType.equalsIgnoreCase("Final") || lStrreqType.equalsIgnoreCase("FW") || lStrreqType.startsWith("F")){
					rowList = new ArrayList<Object>();
					rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENHEADER1FW"),centerAlign));
					dataList.add(rowList);
				}
				//Ended By vivek Sharma 07 June 2017
*/				rowList = new ArrayList<Object>();
				/*rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE1A"),centerAlign));*//*commented brijoy 16012019*/
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE1A"),CenterBoldAlignVO));
				dataList.add(rowList);
				...
				dataList.add(rowList);*/ //commented by brijoy 15012019	
				
				rowList = new ArrayList<Object>();
				/*rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE2Achn"),centerAlign));*//*commented brijoy 16012019*/
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE2Achn"),CenterBoldAlignVO));
				dataList.add(rowList); //added by brijoy 15012019
				
				rowList = new ArrayList<Object>();
				rowList.add(new StyledData("("+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE2A2chn")+")",centerAlign));
				dataList.add(rowList); //added by brijoy 15012019
				
				rowList = new ArrayList<Object>();
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE2A22chn"),centerAlign));
				dataList.add(rowList); //added by brijoy 15012019
				...
				rowList = new ArrayList<Object>();
				/*rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE4A"),centerAlign));*//*commented brijoy 16012019*/
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE4Achn")+space(30)+"Barcode",centerAlign));
				dataList.add(rowList);
				...
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE5A")+strDdoCode+" ]",strTrsryCode+space(12)+strTrsryName+space(40)),leftAlign));
				dataList.add(rowList);*/

				rowList = new ArrayList<Object>();
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINETRSRY")+" [ "+strTrsryCode+" "+strTrsryName+" ]"+space(18)+""+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE5A")+strDdoCode+" ]",leftAlign));
				dataList.add(rowList);
				...
				rowList = new ArrayList<Object>();
				/*rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE11A"),leftAlign));*/ //coomented by brijoy 16012019
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE11Achn")+space(30)+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE11A1chn"),leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE15A"),""+space(40)+"[Code]"+space(40)+"[description]"),
						leftAlign));*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE15A"),""+space(40)+" "+space(40)+""),
						leftAlign));  /*added by brijoy 16012019*/
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE16A"),""+space(49)+"[Code]"+space(40)+"[description]"),
						leftAlign));*/ /*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE16A"),""+space(49)+""+space(40)+""),
						leftAlign));
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE17A"),""+space(51)+"[Code]"+space(40)+"[description]"),
						leftAlign));*/ /*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE17A"),""+space(51)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE18A"),""+space(52)+"[Code]"+space(40)+"[description]"),
						leftAlign));*/ /*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE18A"),""+space(52)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE19A"),""+space(56)+"[Code]"+space(40)+"[description]"),
						leftAlign));*/ /*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE19A"),""+space(56)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE20A"),""+space(43)+"[Code]"+space(40)+"[description]"),
						leftAlign));*//*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE20A"),""+space(43)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE21A"),""+space(31)+"[Code]"+space(40)+"[description]"),
						leftAlign));*//*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE21A"),""+space(31)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
				...
				/*rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE22A"),""+space(39)+"[Code]"+space(40)+"[description]"),
						leftAlign));*//*commented by brijoy 16012019*/
				rowList.add(new StyledData( MessageFormat.format(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE22A"),""+space(39)+""+space(40)+""),
						leftAlign));
				dataList.add(rowList);
              ...
              //Work on it
				...
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE23A")+"["+space(7)+lStrSanctionedAmount+space(7)+"]"+space(25)+""+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE23Aa")+space(15)+"["+space(14)+"NA"+space(14)+"]",leftAlign));  //Added Amount Value by Vivek 08 june 2017 
				dataList.add(rowList);

				rowList = new ArrayList<Object>();
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE24A")+space(7)+"["+space(25)+"]"+space(25)+""+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE24Aa")+space(30)+"]",leftAlign));
				dataList.add(rowList);
				...
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE25A")+space(9)+"["+space(7)+lStrSanctionedAmount+space(7)+"]"+space(25)+""+gObjRsrcBndle.getString("GPF.PAYBILLGENLINE25Aa")+space(21)+"["+space(14)+"NA"+space(14)+"]",leftAlign)); //Added Amount Value by Vivek 08 june 2017
				dataList.add(rowList);
				...
				rowList = new ArrayList<Object>();
				rowList.add(new StyledData(gObjRsrcBndle.getString("GPF.PAYBILLGENLINE38A")+" "+strOrderMnth,leftAlign));
				dataList.add(rowList);
				...
				td1.add(new StyledData((lStrName+" "+lStrDesignation),centerAlign1));
				td1.add(new StyledData((lStrSevaarthId),centerAlign1));
				td1.add(new StyledData((lStrGpfAccNo),centerAlign1));
				td1.add(new StyledData((lStrOrderNumber),centerAlign1));	
			...
			if(report.getReportCode().equals("80000691"))
			{
				...
				if(!BankDetails.isEmpty() && BankDetails!= null) {
					...
					if(lObjBankDetails[3] != null){
						lStrEACCNo = lObjBankDetails[3].toString();
					}
				...
				//td1.add(new StyledData(lStrGpfAccNo,centerAlign1));  
				td1.add(new StyledData(lStrEACCNo,centerAlign1));   //Added By Vivek 15 June 2017
				td1.add(new StyledData(lStrSanctionedAmount,centerAlign1));	
				dataList1.add(td1); 
			}
...
//Added By Vivek Sharma 10 July 2017
				//
				gLogger.error(" lLngPostId : " + lLngPostId);
				gLogger.error(" gStrPostId : " + gStrPostId);
				//String lStrDDOCodeOfLoggedInDDO = lObjGPFRequestProcess.getDdoCodeForDDO(lLngPostId);
				String lStrDDOCodeOfLoggedInDDO = lObjGPFRequestProcess.getDdoCodeForDDO(gStrPostId); // added by brijoy 28022019
				String ddocode = "7101";
				...
				rowList.add(new StyledData(lStrEmployerDsgnName,rightAlign));   //Added By Vivek Sharma
						
				dataList1.add(rowList);

				rowList = new ArrayList<Object>();
				//rowList.add(new StyledData(gObjRsrcBndle.getString("PRAMANPATRA.LINE8A"),rightAlign));
				rowList.add(new StyledData(lStrEmployerOfficeName,rightAlign));   //Added By Vivek Sharma
				dataList1.add(rowList);
	...
	public List getEmployeeDetails(String lStrGpfAccNo, String lStrTransactionId, String lStrreqType)
	{
		...
		try
		{
			...
			else if (lStrreqType.equalsIgnoreCase("Final") || lStrreqType.equalsIgnoreCase("FW") || lStrreqType.startsWith("F"))
			{
				...
				lSBQuery.append(" , PAYABLE_AMOUNT+1 FROM MST_DCPS_EMP ME JOIN MST_EMP_GPF_ACC MEGA ");
				lSBQuery.append("ON MEGA.MST_GPF_EMP_ID = ME.DCPS_EMP_ID  ");
				lSBQuery.append("JOIN ORG_DESIGNATION_MST ODM ON ME.DESIGNATION = ODM.DSGN_ID ");
				lSBQuery.append("JOIN TRN_GPF_FINAL_WITHDRAWAL MGA ON MEGA.GPF_ACC_NO = MGA.GPF_ACC_NO AND MEGA.GPF_ACC_NO  = :gpfAccNo AND MGA.TRANSACTION_ID  = :transactionId ");
				lSBQuery.append("join org_ddo_mst ddo on ddo.DDO_CODE = me.DDO_CODE ");
				lSBQuery.append("join CMN_LOCATION_MST loc on loc.LOC_ID = substr(me.ddo_Code,0,4) ");
			}
			...
			if ((!(lStrreqType.equalsIgnoreCase("FW"))) || (!(lStrreqType.equalsIgnoreCase("Final"))) || (!(lStrreqType.startsWith("F")))) {
				lQuery.setParameter("reqtype", lStrreqType);
			}
	...
	public List getOrderNoFromGPFActNoAndTrsnID( String lStrTransactionId)
	{
		...
		try{
			StringBuilder lSBQuery = new StringBuilder();
			lSBQuery.append("SELECT req.ORDER_No,bill.BILL_NO,MONTHNAME(ORDER_DATE) || ' ' || YEAR(ORDER_DATE) FROM mst_gpf_req req inner join MST_GPF_BILL_DTLS bill on req.TRANSACTION_ID = bill.TRANSACTION_ID where req.TRANSACTION_ID = :transactionId");
			/*lSBQuery.append("SELECT req.ORDER_No,bill.BILL_NO,MONTHNAME(ORDER_DATE) FROM mst_gpf_req req inner join MST_GPF_BILL_DTLS bill on req.TRANSACTION_ID = bill.TRANSACTION_ID where req.TRANSACTION_ID = :transactionId");*/ /*commented by brijoy 16012019*/
			Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
	...
	//Added By Vivek Sharma 07 June 2017
	public List getTreasuryNameandCodeOfEmp(String lStrDDOCode) {
		List<String> lLstTreasury = null;
		String lStrTreasuryName = "";
		try {
			StringBuilder lSBQuery = new StringBuilder();			
			lSBQuery.append("SELECT CLM.locName,CLM.locId FROM RltDdoOrg RDO,CmnLocationMst CLM WHERE RDO.ddoCode = :ddoCode AND RDO.locationCode = CLM.locId");			
			Query lQuery = ghibSession.createQuery(lSBQuery.toString());
